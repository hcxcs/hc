...
const bluetoothLoad = inputValue => {
  // BluetoothDetail.inputValue='HC640'
    ...
    success: function (res) {
      ...
      if (res.devices.length>0){
        for (var i = 0; i < res.devices.length; i++) {
          if(res.devices[i].name=="未知设备")continue;
          if(BluetoothDetail.inputValue!="") {
            if (res.devices[i].name == BluetoothDetail.inputValue) {
      ...
      BluetoothDetail.devices=devices;
